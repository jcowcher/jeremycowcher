...
- **HTML-post chrome is injected, not authored (single-sourced):** `build.js`'s `injectChrome()` replaces the shared chrome in every HTML post at build time with the canonical versions used by all other pages, so a chrome edit lives in exactly one place and propagates everywhere on the next build (no per-post edits). What gets injected: two **marker comments** in the `<head>` — `<!--gk:tokens-->` becomes `<style>${TOKENS}</style>` (`@gemka/core` tokens) and `<!--gk:style-->` becomes `<style>${STYLE}</style>` (the site stylesheet) — plus the `<nav>` (from `NAV`), the `.footer-links` footer (from `FOOTER_LINKS`), and the clock + mobile-menu `<script>` (from `SCRIPT`). **What the author owns (passes through untouched):** the `<!--post-->` header, the `<head>` `<title>`/meta/OG/fonts, the `<main class="post">` body, and **every real `<style>` block** — there is no longer a "1st/2nd is chrome, 3rd-onward is bespoke" rule; the two chrome stylesheets are markers now, so any `<style>` block in a post source is the author's own bespoke CSS. Authoring rule: copy an existing HTML post as the scaffold, keep the two `<!--gk:tokens-->` / `<!--gk:style-->` markers in the head, and edit only the author-owned parts. Structural contract enforced by build-time assertions (build fails, naming the slug, otherwise): the page has **exactly one** `<!--gk:tokens-->` marker, one `<!--gk:style-->` marker, one `<nav>`, one `.footer-links` footer, and one `<script>`.
...
- **About page:** `/about` (full nav, post-page layout), `aboutBody` in `build.js`. Bio in sections: "Current", "Before that" and "The rest" are standalone `<h3>` headings (1.2rem Fraunces, the shared `.post-body h3` style, so all three match); "Reach" stays a run-in `<strong>` at the start of its paragraph because it is a contact line, not a bio section. Linked from `.nav-right` immediately before the GitHub icon, plus a mobile-menu item after The Why. No date line, no post disclaimer. Copy sourced from Jeremy's CV and the GemKa welcome-email product blurbs; edits to it are content changes Jeremy signs off on.
...
- **Palette via GemKa tokens:** `style.css` consumes the `--gk-*` tokens directly. There used to be a legacy alias layer (`--black`, `--white`, `--orange`, `--gray-*` declared as thin aliases onto `--gk-*`) so the token migration would not have to touch every call site; that layer is now gone and every call site references the token. The alias names had become misleading, which is why the scale was retired: `--orange` was oxblood (`var(--gk-accent)`), `--black` was warm ink (`--gk-ink`), `--white` was cream paper (`--gk-paper`), and the gray scale had collapsed so `--gray-300` and `--gray-400` both resolved to `--gk-faint`, while `--gray-100` was a fill (`--gk-surface-2`), not a border. Page background uses `--gk-bg`, and there is still no pure white, black, or orange anywhere. Note: files under `posts/_drafts` and `posts/_archive` still reference the old aliases, but they are not built, and their token and style blocks get replaced with the gk markers when a draft is promoted, so they need no change now.

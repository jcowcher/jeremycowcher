...
My code has two branches: dev is my working draft and main is the final, live version.

I used Claude Code to set this up so all my day-to-day work happens on dev by default. I write and edit code in VS Code, which is my IDE (basically a text editor built for coding).

I keep three reference files in each project. CLAUDE.md is the technical spec — Claude Code reads it automatically every time it starts so it knows how the project is built. CONTEXT.md is the product brief — what the project is, who it's for, how it works. NOTES.md is my troubleshooting journal — non-obvious decisions, workarounds, things that bit me. Claude Code created all three and updates them when I ask.
